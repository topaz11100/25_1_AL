...
$small-o$의 정의
$$
o(f(x)) = \{ g(x) \ | \ \forall c>0,\quad \exists N>0,\quad \forall x>N,\quad g(x) \le cf(x) \}
$$
...
$$
$$
\rightarrow \forall \epsilon>0,\quad \exists \delta>0,\quad \forall x>\delta,\quad g(x) \le \epsilon f(x)
$$
$$
...
비동차항이 상수항이므로 특수해는 상수형태 가정, 특수해 $-1$가정하고 재귀관계에 대입 시
$$
...
$$
가정에 의해 $n = 3^k ,\quad  T(3^k) = 3T(3^{k-1}) ,\quad t(k) = T(3^k)$ 라 하자 그럼 $t(k) = 3t(k-1)$가 성립한다
...
$O(n)$으로 중앙값을 구하고 분할한뒤 반으로 쪼개진다 분할 시간은 배열 상태와 상관 없이 배열의 크기이므로
...
따라서 $c=c_0 ,\quad N=2 ,\quad \forall n>N ,\quad B(n) \le c_0nlog_{2}n$ 이므로 
