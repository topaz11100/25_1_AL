...
$$
\lim_{x \to  \infty} f(x) = L \Leftrightarrow \forall \epsilon>0, \exists \delta>0 \quad x>\delta \rightarrow |f(x)-L|<\epsilon
$$
...
보조정리 2 $x>N \text{(양수인 상수) 일때 } 0 < f(x) < g(x)$ 면 $\lim_{x \to \infty} g(x) = 0 \rightarrow \lim_{x \to \infty} g(x) = 0$
보조정리 2 증명 :
$$
\lim_{x \to \infty} g(x) = 0 \rightarrow \forall \epsilon>0, \exists \delta>0 \quad x>\delta \rightarrow g(x)<\epsilon
$$

$$
n = max(\delta, N) \text{라 하면 다음이 성립한다}
$$

$$
\lim_{x \to \infty} g(x) = 0 \rightarrow \forall \epsilon>0, \exists \delta>0 \quad x > n \geq \delta \rightarrow 0 < f(x) < g(x)<\epsilon
$$

$$
\rightarrow \forall \epsilon>0, \exists n>0 \quad x > n \rightarrow 0 < f(x) < \epsilon \rightarrow \lim_{x \to \infty} f(x) = 0
$$

보조정리 3 : 자연수 $ n \geq 10 \rightarrow n^3 < 2^n $
보조정리 3 증명:
수학적 귀납법으로 증명한다
$$
n = 10 \rightarrow 1000 < 1024 \text{ 부등식 성립} 
$$

$$
n = k \text{(k > 10) 에서 성립 가정} 
$$

$$
n = k+1 \rightarrow (k+1)^3 = k^3+3k^2+3k+1 < k^3+3k^2+3k^2+3k^2
$$

$$
= k^3+9k^2 < 2 \cdot k^3 < 2 \cdot 2^k \text{(귀납가설)}= 2^{k+1}
$$


보조정리 4 : $g(x)>0, f(x)>0$ 일 때 $\lim_{x \to \infty} \frac{g(x)}{f(x)} = 0 \rightarrow g(x) \in o(f(x))$
보조정리 4 증명 : 

$$
\lim_{x \to  \infty} \frac{g(x)}{f(x)} = 0
\rightarrow \forall \epsilon>0, \exists \delta>0 \quad x>\delta \rightarrow \frac{g(x)}{f(x)}<\epsilon \text{ (엡실론 델타 논법)}
$$

$$
\rightarrow \forall \epsilon>0, \exists \delta>0, \forall x>\delta \quad g(x)<\epsilon f(x) \text{ (보조정리 1)}
$$

$$
\rightarrow \forall \epsilon>0, \exists \delta>0, \forall x>\delta \quad g(x) \le \epsilon f(x)
...
$$
n > 10 \rightarrow \frac{n^2}{2^n} < \frac{n^2}{n^3} \text{ (보조정리 3)} ,\quad \lim_{n \to \infty} \frac{n^2}{n^3} = \lim_{n \to \infty} \frac{1}{n} = 0 \rightarrow \lim_{n \to  \infty} \frac{n^2}{2^n}=0 \text{ (보조정리 2)}
$$

$$
\lim_{n \to  \infty} \frac{n^2}{2^n}=0 \text{ 이므로 보조정리 4에 의해 } n^2 \in o(2^n)
$$
